...
function montarlista(vetor) {
  vetor.forEach((e) => {
    ...
    form.appendChild(data_prevista)
    form.appendChild(data_devolucao)
    ...
    function alterarItem(id_emprestimo) {
      ...
      if (data_prevista.value) {
        dadosPATCH.data_prevista = data_prevista.value;
      }

      if (data_devolucao.value) {
        dadosPATCH.data_devolucao = data_devolucao.value;
      }

      if (valor.value) {
        dadosPATCH.valor = valor.value;
      }
